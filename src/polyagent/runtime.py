...
class TaskRuntimePaths:
    ...
    prompt_log_jsonl: Path
    trade_log_jsonl: Path
    status_log_txt: Path


class PolyMonitorRuntime:
    def __init__(self, task_name: str, daemon: bool = False) -> None:
        ...
        self.paths = TaskRuntimePaths(
            ...
            prompt_log_jsonl=task_dir / 'logs' / 'decision_prompts.jsonl',
            trade_log_jsonl=task_dir / 'logs' / 'trades.jsonl',
            status_log_txt=task_dir / 'logs' / 'monitor_status.log',
        )
        self.rag = EventRAG('sentence-transformers/all-MiniLM-L6-v2')

        self.latest_news: dict[str, Any] | None = None
    ...
    def _extract_trade_record(self, response: str) -> dict[str, Any] | None:
        try:
            start = response.find('{')
            end = response.rfind('}')
            if start == -1 or end == -1:
        ...
        if not payload.get('should_trade'):
            return None
        return {
    ...
    def _render_status(self) -> str:
        markets = self._count_jsonl(self.paths.filtered_events_jsonl)
        tweets = self._count_jsonl(self.paths.tweet_jsonl)
        trades = self._count_jsonl(self.paths.trade_log_jsonl)
        heartbeat = '●'

        latest_news_text = 'N/A'
        if self.latest_news:
            latest_news_text = (
                f"[{self.latest_news.get('created_at')}] @{self.latest_news.get('user')}: "
                f"{self.latest_news.get('text', '')}"
            )

        trade_lines = ['(none)'] if not self.recent_trades else [
            f"- {t.get('time')} | {t.get('side')} {t.get('token_id')} ${t.get('amount_usd')}" for t in self.recent_trades[-5:]
        ]

        header = (
            f"{ASCII_LOGO}\n"
            f"POLY MONITOR v1.1.0 | started UTC: {self.started_at.isoformat()}\n"
            f"█▄█▄█▄█▄█▄█▄█▄█▄█▄█\n"
    ...
    async def _heartbeat_loop(self) -> None:
        while True:
            ...
            if self.daemon:
                self.paths.status_log_txt.parent.mkdir(parents=True, exist_ok=True)
                self.paths.status_log_txt.write_text(status, encoding='utf-8')
            else:
    ...
    async def process_news(self, tweet: dict[str, Any]) -> None:
        ...
        trade = self._extract_trade_record(result.response)
        if trade:
            trade_row = {'time': datetime.now(timezone.utc).isoformat(), 'task_name': self.task_name, **trade}
            self._append_jsonl(self.paths.trade_log_jsonl, trade_row)
            self.recent_trades.append(trade_row)
        ...
        self._append_jsonl(
            self.paths.runtime_log_jsonl,
            {'time': datetime.now(timezone.utc).isoformat(), 'type': 'decision', **decision_summary},
        )
    ...
    async def run_forever(self) -> None:
        ...
        await asyncio.gather(refresh_loop(), self.twitter_loop(), self._heartbeat_loop())
